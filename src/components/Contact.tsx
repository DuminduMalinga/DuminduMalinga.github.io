...
const EMAILJS_SERVICE_ID = 'service_vul7qot';
const EMAILJS_TEMPLATE_ID = 'template_zfzkedn';
const EMAILJS_PUBLIC_KEY = 'Oa2igSQoOVGbKMLvg';
// ────────────────────────────────────────────────────────────────────────────
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFb({ type: 'success', msg: `Message sent! Thanks ${form.name}, I'll get back to you soon.` });
      setForm({ name: '', email: '', subject: '', message: '' });
    } catch (err: unknown) {
      console.error('EmailJS error:', err);
      const detail = (err as { text?: string; status?: number })?.text ?? String(err);
      setFb({ type: 'error', msg: `Send failed: ${detail}` });
    } finally {
      setSending(false);
    }
  };
