...
const PLATFORM_META: Record<string, { icon: string; label: string }> = {
  flutter: { icon: '📱', label: 'Flutter' },
  android: { icon: '🤖', label: 'Android' },
  web: { icon: '🌐', label: 'Web' },
  python: { icon: '🐍', label: 'Python' },
};
const getPlatform = (filterTags: string[]) => {
  const key = filterTags.find(t => PLATFORM_META[normalizeFilter(t)]);
  return key ? PLATFORM_META[normalizeFilter(key)] : null;
};
    ...
    <section id="projects" ref={sectionRef} className="section" aria-labelledby="projects-heading">
      <div className="container">
        ...
        <div className="projects-grid">
          {filtered.map((proj) => {
            const platform = getPlatform(proj.filterTags);
            return (
            <article key={proj.title} className="project-card glass-card">
              <div className="project-thumb">
                ...
                {platform && (
                  <span className="project-platform">{platform.icon} {platform.label}</span>
                )}
              ...
              <div className="project-body">
                ...
                <div className="project-footer">
                  {proj.links.map(link => (
                    <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" className="project-cta-btn">
                      <GithubIcon size={14} /> {link.label}
                      <ArrowIcon />
                    </a>
                  ))}
